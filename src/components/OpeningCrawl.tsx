...
const INTRO_DURATION_MS = 116000;
export const LOGO_SVG = `<svg version="1.0" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
...
function OpeningCrawl({ onComplete }: OpeningCrawlProps) {
  ...
  useEffect(() => {
    ...
    completeRef.current = finish;

    const fadeTimer = window.setTimeout(() => setFadingOut(true), INTRO_DURATION_MS - 1200);
    const doneTimer = window.setTimeout(() => finish(), INTRO_DURATION_MS);
    const skipRevealTimer = window.setTimeout(() => setShowSkipButton(true), 7000);

    // Same star script behavior as requested.
    const numStars = 100;
    const createdStars: HTMLDivElement[] = [];
  ...
  return (
    <div className={`fixed inset-0 ${fadingOut ? 'fade-out' : ''}`}>
      <style>{`
        body {
          margin: 0;
      ...
      <section className="intro">
        Hace mucho tiempo en una institucion del Bienestar,
        <br />
        muy muy lejana....
      </section>

      <section className="logo" aria-hidden="true" dangerouslySetInnerHTML={{ __html: LOGO_SVG }} />

      <div id="board">
        <div id="content" ref={contentRef}>
          <p id="title">EPISODIO XXXIX</p>
          <p id="subtitle">CUMPLEAÑOS ARMANDO</p>
          <br />
          <p>
